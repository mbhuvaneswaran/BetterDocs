...
*/
        ...
        $(document).on('close', '.remodal', function (e) {
            if (e.reason === "confirmation") {
                app.saveConfig($("#elasticSearchURL").val(), $("#resultSize").val(),$("#theme").val());
            }
        });
        ...
        $("#theme").val(getAppProperty("theme"));
        $('#searchString').focus();
    ...
    function enableAceEditor(id, content, lineNumbers) {
        ...
        changeTheme(editor,getAppProperty("theme"));
    ...
    function updateConfig(url, size,theme) {
        esURL = url.trim();
        resultSize = size;
        if(theme!=getAppProperty("theme")){
            //set theme;
            setAppProperty("theme",theme);
    ...
    function changeThemeAll(theme){
        ...
        for(var i=0;i<editors.length;i++){
            ...
            editors[i].editor.setTheme("ace/theme/"+theme);
        }
    ...
    function initiateLocalStorage(){
        var appDetails=localStorage.getItem("BetterDoc");
        appDetails?"":((appDetails=JSON.stringify({"theme":"github"}))&&(localStorage.setItem("BetterDoc",appDetails)));

        var loc=appDetails;
        appDetails=JSON.parse(appDetails);
        app.theme=appDetails.theme;
        return loc;
    }
function getAppProperty(prop){
    var loc=localStorage.getItem("BetterDoc");
    if(!loc)
        loc=initiateLocalStorage();

    var appDetails=JSON.parse(loc);
    return appDetails[prop];
}
    function setAppProperty(prop,value){
        var appDetails=JSON.parse(localStorage.getItem("BetterDoc"));
        appDetails[prop]=value;
        localStorage.setItem("BetterDoc",JSON.stringify(appDetails))
    }
...
function removeEditor(index){
    return _.reject(editors, function(item) {
        return item.id === index+"-editor";
    });
}
    ...
    Mousetrap.bind('shift+f1', function(e) {
        //updateConfig();
        alert("help");
